        ...
        if ( prc->fCubeful && GetDPEq ( NULL, &rDP, pci ) &&
          ...
          case REDOUBLE_TAKE:
			if (log_rollouts) {
				...
				/* update statistics */
			if( aarsStatistics )
				(void)MT_SafeInc(&aarsStatistics[ ici ][ pci->fMove ].acDoubleTake[ LogCube ( pci->nCube ) ]); 
...
extern void RolloutLoopMT(void)
{
  ...
  /* ============ begin rollout loop ============= */
	...
	while ((i = MT_SafeInc(&ro_NextTrail) - 1) < cGames)
	{
    ...
    for (alt = 0; alt < ro_alternatives; ++alt)
	{
      ...
      memcpy ( &aanBoardEval, ro_apBoard[ alt ], 
               sizeof( aanBoardEval ));
    ...
    */
	...
	prc = &ro_apes[alt]->rc;
      ...
      qsort ((void *) ajiJSD, ro_alternatives, sizeof (jsdinfo), 
             comp_jsdinfo_equity);
