...
extern void BearoffStatus(const bearoffcontext *pbc, char *sz)
{
	...
	{
		if (pbc->p)
			title = _("In memory 2-sided exact %d-chequer Hypergammon database evaluator");
		else
			title = _("On disk 2-sided exact %d-chequer Hypergammon database evaluator");

		sprintf(buf, title, pbc->nChequers);
	}
	else
	{
		if (pbc->p)
			title = _("In memory %d-sided bearoff database evaluator");
		else
			title = _("On disk %d-sided bearoff database evaluator");

		sprintf(buf, title, BEAROFF_ONESIDED ? 1 : 2);
	}
	sz += sprintf(sz, " * %s\n", buf);

	sz += sprintf(sz, "   - %s\n", _("generated by GNU Backgammon"));

	detail = _("up to %d chequers on %d points (%d positions) per player");
	sprintf(buf, detail, pbc->nChequers, pbc->nPoints, Combination(pbc->nChequers + pbc->nPoints, pbc->nPoints));
	sz += sprintf(sz, "   - %s\n", buf);
